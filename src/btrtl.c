...
/* RTL8852BD (rom_version 3) needs an "eco4" firmware patch that the standard
 * flat 0xfc20 download path cannot load: it is a multi-record image, each
 * record loaded to its own address via a 0xfc62 setup command. The sequence
 * below was reverse-engineered from the Realtek Windows driver. Firmware file
 * "rtl_bt/rtl8852bd_eco4.bin" layout: "E4RD", __le32 num_records, then per
 * record __le32 load_addr, __le32 len, <len> bytes. Records are sent in file
 * order (auxiliary records first, the checksummed main record last); only the
 * final fragment of the final record carries the 0x80 end bit, then an empty
 * 0xfc20/0x80 commits.
 */
#define RTL_ECO4_FRAG 252

/* 0xfc62 is a write-memory command: [0x21][dst:le32][value:le32] (the write twin
 * of 0xfc61 read). 0x801200cc holds the address the chip streams 0xfc20 fragments
 * into. Writing a record's load address there before sending its fragments is what
 * steers each record to its own location; without it every record lands on the
 * default buffer (0x8010f720) and the patch never launches.
 */
#define RTL_ECO4_BUFPTR 0x801200ccU

static int rtl_eco4_cmd(struct hci_dev *hdev, __u16 op, const __u8 *buf, int len)
{
	struct sk_buff *skb;
	int status;
			     ...
			     struct btrtl_device_info *btrtl_dev)
{
	...
	if (ret < 0) {
		rtl_dev_err(hdev, "eco4: rtl8852bd_eco4.bin not found");
		return ret;
	}
	...
	for (i = 0; i < nrec; i++) {
		...
		data = p;
		p += len;		/* advance by the on-file record length */


		/* point the download buffer at this record's load address */
		setup[0] = 0x21;
		...
		if (status != 0) {
			rtl_dev_err(hdev, "eco4: buffer redirect rejected (0x%02x)",
				    status);
			ret = -EIO;
			goto out_frag;
		}
		...
		for (off = 0; off < len; off += RTL_ECO4_FRAG) {
			...
			/* plain indices only -- bit 7 on a data fragment is not a
			 * commit here and wedges the chip; the standalone
			 * 0xfc20/0x80 below is the commit
			 */
			frag[0] = idx;
	...
	/* commit the complete firmware */
	{
		u8 fin = 0x80;

		rtl_eco4_cmd(hdev, 0xfc20, &fin, 1);
	}
				...
				struct btrtl_device_info *btrtl_dev)
{
	...
	/* No eco-4 snippet exists for this silicon in any linux-firmware release,
	 * so the stock parsers always fail here -- but with different errnos
	 * depending on the container format they are handed:
	 *   v1 "Realtech" EPATCH -> -EINVAL  ("didn't find patch for chip id 3")
	 *   v2 "RTBTCore"        -> -ENODATA
	 * Accept either; the ic_info checks below keep this scoped to RTL8852BD.
	 */
	if ((ret == -ENODATA || ret == -EINVAL) && btrtl_dev->ic_info &&
	    btrtl_dev->ic_info->lmp_subver == RTL_ROM_LMP_8852A &&
	    btrtl_dev->ic_info->hci_rev == 0xb &&
	    btrtl_dev->rom_version == 3) {
		rtl_dev_info(hdev,
			     "RTL8852BD: using multi-record eco4 download");
		ret = rtl_download_eco4(hdev, btrtl_dev);
		goto out;
	}
	if (ret < 0)
		goto out;
